Public Class itmDiffusal_Blade_1
  Inherits ItemBase


  Public Sub New(theparent As idisplayunit, parentorbuildname As String)
    MyBase.New(theparent, parentorbuildname)
    mName = eItemname.itmDIFFUSAL_BLADE_1
    ...
    mMadeFromItems.Add(eItemname.itmRECIPE_DIFFUSAL_BLADE_1)
    mMadeFromItems.Add(eItemname.itmBLADE_OF_ALACRITY)
    mMadeFromItems.Add(eItemname.itmBLADE_OF_ALACRITY)
    mMadeFromItems.Add(eItemname.itmROBE_OF_THE_MAGI)

    mBuildsToNames.Add(eItemname.itmDIFFUSAL_BLADE_2)
    ...
    Description = "Active: Purge - Purges a target unit, removing its buffs, and slowing it. Instantly kills summoned units.|Passive: Manabreak - Your attacks burn 25 mana and deal the same amount in physical damage. Does not stack with other manabreak abilities.|Upgradable."
  ...
  Public Overrides Function GetPassiveModifiers(thestateindex As Integer, thegame As dGame, theitem_InfoID As iUnitUpgrade, _
    ...
    Dim intmod As New Modifier(agival, passiveselfinfo)
  ...
  Public Overrides Function GetActiveModifiers(thestateindex As Integer, thegame As dGame, theitem_InfoID As iUnitUpgrade, _
    ...
    Dim buffremval As New modValue(1, eModifierType.RemoveBuffs, occurencetime)

    Dim buffremove As New Modifier(buffremval, unittargetenemytargetinfo)

    outmods.Add(buffremove)


    'this is a guess. haven't found actual number
    Dim purgeslowval As New modValue(0.1, eModifierType.MoveSpeedPercentSubtracted, occurencetime)

    Dim purgeslow As New Modifier(purgeslowval, unittargetenemytargetinfo)
    ...
    Dim purgedamage As New Modifier(purgedamval, unittargetenemytargetinfo)

    outmods.Add(purgedamage)


    Dim mandrainval As New modValue(20, eModifierType.ManaSubtracted, occurencetime)

    Dim manadrain As New Modifier(mandrainval, unittargetenemytargetinfo)

    outmods.Add(manadrain)


    Dim manadamageval As New modValue(20, eModifierType.DamagePhysicalAdded, occurencetime)

    Dim manadraindamage As New Modifier(manadamageval, unittargetenemytargetinfo)
