Public Class ctrlSummaryDual
  Implements iGraphable

  Private ctrlDataItemViewer As ctrlSelectedDataitemDualViewer
  ...
  Private mSummaryDataitem As ctrlSwatchDataItemLabelDual
  Public Sub Load(radsummarydataitem As iDataItem, diresummarydataitem As iDataItem, _
                  Raddataitems As List(Of List(Of iDataItem)), Diredataitems As List(Of List(Of iDataItem)), _
    ...
    grphSummary.Load(Raddataitems, Diredataitems, _
                     subheadingtitles, barwidth, margin, game, _
                     New SolidColorBrush(PageHandler.dbColors.RadiantColor),
                     New SolidColorBrush(PageHandler.dbColors.DireColor), _
                     New SolidColorBrush(PageHandler.dbColors.NeutralTeamColor))



    ctrlDataItemViewer = New ctrlSelectedDataitemDualViewer

    ctrlDataItemViewer.Load(headingtextbrush, bodytextbrush, thefontsize, verticalorientaton, game)

    mSummaryDataitem = New ctrlSwatchDataItemLabelDual(game.TimeKeeper.CurrentTime, _
                                                       radsummarydataitem, diresummarydataitem, _
                                                       headingtextbrush, bodytextbrush, _
                                                                  New SolidColorBrush(radsummarydataitem.MyColor), _
                                                                  Constants.cHeadingFontSize, radsummarydataitem.DisplayName, "", _
                                                                  New SolidColorBrush(radsummarydataitem.MyColor), _
                                                                  New SolidColorBrush(diresummarydataitem.MyColor))
    ...
    For i = 0 To Raddataitems.Count - 1
      Dim curradlist = Raddataitems.Item(i)
      Dim curdirelist = Diredataitems.Item(i)
      For x = 0 To curradlist.Count - 1
        ctrlDataItemViewer.AddDataitem(curradlist.Item(x), curdirelist.Item(x))
      Next
  ...
  Private Sub grphSummary_MouseLeave(sender As Object, e As MouseEventArgs)
    If Not grphSummary.SelectedSummmaryBar Is Nothing Then
      ctrlDataItemViewer.SetVisibleAttribute(ctrlDataItemViewer.GraphSelectedDataItemViewer.DireDataitem)
    End If
  End Sub
